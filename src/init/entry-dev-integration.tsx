 ...
 */
export function createRadixApiUrl(path: string, protocol: string = window.location.protocol): string {
  return `${protocol}//${window.location.host}/api/v1${path}`
}

fetchMock.post('*', { thisIsADummyResponse: true })

class IntegrationComponent extends Component<{ component?: string }, { content: React.JSX.Element }> {
    ...
    this.fetchModule(props.component)
      .then((module) => {
        if (this.isLoaded && module) {
          ...
          const mockServerRR = new Server(`${createRadixApiUrl('radixregistrations', 'wss:')}?watch=true`)
          const mockServerRA = new Server(`${createRadixApiUrl('radixapplications', 'wss:')}?watch=true`)
      ...
      .catch(
        () =>
          ...
          this.setState({
            content: (
              <p>
                The file "integration.jsx" or "integration.tsx" does not exist for the component{' '}
...
const testPathMatch = window.location.pathname.match(RegExp(`^${routes.devIntegration}`))
