...
const ApplicationAlerting = ({ appName }: Props) => {
  ...
  const {
    ...
    refetch,
    ...state
  } = radixApi.useGetApplicationAlertingConfigQuery({ appName }, { pollingInterval })

  const [enableAlertMutation, { isLoading: savingEnable }] = radixApi.useEnableApplicationAlertingMutation()
  const [disableAlertMutation, { isLoading: savingDisable }] = radixApi.useDisableApplicationAlertingMutation()
  const [updateAlertMutation, { isLoading: savingUpdate }] = radixApi.useUpdateApplicationAlertingConfigMutation()
