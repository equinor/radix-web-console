...
export function PipelineFormBuildBranches({ onSuccess, application, pipelineName }: FormProp) {
  ...
  const [triggerBuildDeploy, buildDeployState] = useTriggerPipelineBuildDeployMutation()
  ...
  const handleOnTextChange = ({ target: { value } }: ChangeEvent<HTMLInputElement>) => {
    ...
    const cleanedBranches = Object.entries(branches).map(([key, value]) => [cleanBranch(key), value])
    ...
    const filter = cleanedBranches.filter(([key]) => {
      if (typeof key === 'string') {
        try {
          ...
          console.error(`Error processing branch regex template: ${key} for the value: ${buildBranch}`, e)
          return false
        }
      }
      return false
  ...
  return (
    <form onSubmit={handleSubmit}>
      <fieldset disabled={createJobState.isLoading} className="grid grid--gap-medium">
        {hasEnvironments && hasBranches ? (
          <div className="grid grid--gap-small input">
            ...
            >
              {isBuildDeployPipeline ? <>Build and deploy a git branch</> : <>Build (but do not deploy) a git branch</>}
            </Typography>
            ...
            {!isAnyValidRegex(buildBranch) && buildBranch && filteredBranches?.length > 0 && (
              <div className="grid grid--gap-small input">
                <Typography group="input" variant="text" token={{ color: 'currentColor' }}>
                ...
                <NativeSelect
                  ...
                  value={toEnvironment}
                >
                  <option value="">All environments build from the branch {buildBranch}</option>
            ...
            {isBuildDeployPipeline && buildBranch && !isAnyValidRegex(buildBranch) && (
              <TargetEnvs targetEnvs={toEnvironment ? [toEnvironment] : filteredBranches} branch={buildBranch} />
            )}
          </div>
        ) : (
          <>
              ...
              <>
                {!hasBranches && (
                  <Alert className="icon">
                    ...
                    <div>
                      <Typography>No environments are configured to be built by Radix.</Typography>
                    </div>
                  </Alert>
                )}
              </>
        ...
        <div className="o-action-bar">
          ...
          {createJobState.isError && (
            <Alert type="danger">Failed to create job. {getFetchErrorMessage(createJobState.error)}</Alert>
          )}
          <div>
