...
export class ConfigureApplicationGithub extends React.Component {
  constructor(props) {
    super(props);
    this.state = { deployOnly: false };
    this.handleSetUseOtherCiTool = this.handleSetUseOtherCiTool.bind(this);
  }
  handleSetUseOtherCiTool() {
    this.setState({ useOtherCiTool: !this.state.useOtherCiTool });
  }
  render() {
    let { app, startVisible, deployKeyTitle, webhookTitle } = this.props;
    return (
            ...
            <fieldset>
              <FormField help={deployOnlyHelp}>
                <input
                  name="deployOnly"
                  type="checkbox"
                  value={this.state.useOtherCiTool}
                  checked={this.state.useOtherCiTool}
                  onChange={this.handleSetUseOtherCiTool}
                />
                Use other CI tool than Radix
              </FormField>
            </fieldset>
          </div>
          {!this.state.useOtherCiTool && (
            <div>
...
const deployOnlyHelp = (
  <span>
    Select this option if your project is hosted on multiple repositories and/or
    requires external control of building. Radix will no longer need a webhook
    and will instead deploy your app through the API/CLI.
    <br />
    See{' '}
    <a href={externalUrls.deployOnlyGuide} target="_blank">
      Deployment Guide
    </a>{' '}
    for details.
  </span>
