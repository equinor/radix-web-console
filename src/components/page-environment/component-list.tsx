...
import { type EnvironmentVulnerabilities, type ImageWithLastScan, scanApi } from '../../store/scan-api'
import { getFetchErrorData } from '../../store/utils/parse-errors'
import { buildComponentMap } from '../../utils/build-component-map'
import { getActiveComponentUrl, getActiveJobComponentUrl, getOAuthReplicaUrl, getReplicaUrl } from '../../utils/routing'
...
function getComponentUrl(appName: string, envName: string, { name, type }: Readonly<Component>): string {
  return type === 'job'
...
function hasComponentOAuth2Service(c: Component): c is Component & { oauth2: OAuth2AuxiliaryResource } {
  return !!c.oauth2
}
...
const ReplicaLinks: FunctionComponent<{
  ...
  replicaList && replicaList.length > 0 ? (
    <div className="component-replica__link-container">
      {replicaList.map((x, i) => (
        <Typography key={i} className="component-replica__link" as={Link} to={urlFunc(x)} link>
...
const EnvironmentComponentScanSummary: FunctionComponent<{
  ...
  scan?.scanSuccess ? (
    ...
    <Typography group="table" variant="cell_text" {...(scan?.scanSuccess === false && { color: 'warning' })}>
      {!scan ? 'No data' : ['Not scanned', 'Scan failed'][+(scan.scanSuccess === false)]}
    </Typography>
  )
...
export const ComponentList: FunctionComponent<ComponentListProps> = ({
  ...
  const [expandedComponents, setExpandedComponents] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(
      components
        ...
        .map((c) => [c.name, !!c.oauth2?.deployments?.some((d) => d.status !== 'Consistent')])
    )
  )
  const [trigger, { data: vulnerabilities, ...vulnerabilityState }] =
    scanApi.endpoints.getEnvironmentVulnerabilitySummary.useLazyQuery()
  const expandComponent = useCallback((name: string) => setExpandedComponents((x) => ({ ...x, [name]: !x[name] })), [])
  ...
  const showChevronColumn = useMemo(() => components.some(hasComponentAdditionalInfo), [components])
  return (
    <>
      {Object.keys(compMap).map((type) => (
        <Accordion className="accordion elevated" chevronPosition="right" key={type}>
                      ...
                      .map(({ x, expanded, i }) => (
                        <Fragment key={i}>
                          ...
                          >
                            ...
                            <Table.Cell>
                              <Typography as={Link} to={getComponentUrl(appName, envName, x)} link>
                                {x.name}
                              </Typography>
                            </Table.Cell>
                            <Table.Cell>
                              <ComponentStatusBadge status={x.status ?? 'Reconciling'} />
                            </Table.Cell>
                            <Table.Cell>
                              <ReplicaLinks
                                replicaList={x.replicaList}
                                urlFunc={(r: ReplicaSummary) => getReplicaUrl(appName, envName, x.name, r.name)}
                              />
                            </Table.Cell>
                            <Table.Cell>
                              {type === 'component' && (
                                ...
                                errorContent={
                                  <samp>
                                    {vulnerabilityState.isError &&
                                      (({ code, message }) => [code, message].filter((x) => !!x).join(': '))(
                                        getFetchErrorData(vulnerabilityState.error)
                                      )}
                                  </samp>
                                }
                              >
                                {vulnerabilities && (
                                  <EnvironmentComponentScanSummary
                                    scan={getEnvironmentComponentScanModel(vulnerabilities, x.name, x.type)}
                                  />
                                )}
                              </AsyncResource>
                            </Table.Cell>
                                      ...
                                      <Table.Cell>
                                        ...
                                        <ReplicaLinks
                                          ...
                                          urlFunc={(r: ReplicaSummary) =>
                                            getOAuthReplicaUrl(appName, envName, x.name, r.name, deployment.type)
                                          }
                                        />
                                      </Table.Cell>
                                      <Table.Cell />
