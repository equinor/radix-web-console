...
export const SecretOverview = ({ appName, componentName, envName, secretName, onSave }: Props) => {
  const { data, refetch, ...envState } = useGetEnvironmentQuery(
    { appName, envName },
    { skip: !appName || !envName, pollingInterval }
  )

  const [trigger, { isLoading }] = radixApi.endpoints.changeComponentSecret.useMutation()
