...
import { isEventObsolete, isEventResolved, isWarningEvent } from '../../utils/event-model'
...
const WarningState: FunctionComponent<{ event: Readonly<Event> }> = ({ event }) => {
...
export const EventSummary: FunctionComponent<EventSummaryProps> = ({ event }) => (
  <Table.Row>
    <Table.Cell>
      <RelativeToNow time={event.lastTimestamp} titlePrefix="Start" capitalize />
    </Table.Cell>
    <Table.Cell>
      <div className="type">
        <GenericStatusBadge type={isWarningEvent(event) ? 'warning' : 'success'}>{event.type}</GenericStatusBadge>
